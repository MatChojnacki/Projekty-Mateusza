...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
      ...
      // Given
        User user1 = new User("user1", "Mateusz");
        User user2 = new User("user2", "Ela");

        Task task1 = new Task("Task 1", "clean", user1, user2, LocalDate.now(), LocalDate.now().plusDays(5));
        Task task2 = new Task("Task 2", "learn", user1, user2, LocalDate.now(), LocalDate.now().plusDays(3));
        Task task3 = new Task("Task 3", "eat", user1, user2, LocalDate.now(), LocalDate.now().plusDays(7));
        ...
        double averageTime = taskLists.stream()
                .filter(taskList -> taskList.getName().equals("In Progress"))
                .flatMap(taskList -> taskList.getTasks().stream())
                .mapToLong(task -> task.getDeadline().toEpochDay() - task.getCreated().toEpochDay())
                .average()
                .orElse(0);

        // Then
        assertEquals(5.0, averageTime);
    }
}